    ...
    if st.button("Generate Certificate", type="primary", use_container_width=True):
        with st.spinner("Generating certificate..."):
            ...
            st.session_state.last_cert_data = {
                ...
                "warranty_years": w_years,
                "warranty_miles": w_miles,
                "warranty_soh_threshold": selected_spec.get("warranty_soh_threshold", 70) if selected_spec else 70,
                "battery_usable_kwh": selected_spec.get("battery_usable_kwh", man_battery if manual_spec else "N/A") if selected_spec else (man_battery if manual_spec else "N/A"),
                "battery_gross_kwh": battery_gross,
                "ac_charge_kw": selected_spec.get("ac_charge_kw", selected_spec.get("charge_rate_ac_kw", "N/A")) if selected_spec else "N/A",
                "dc_charge_kw": selected_spec.get("dc_charge_kw", selected_spec.get("charge_rate_dc_kw", "N/A")) if selected_spec else "N/A",
                "cert_ref": cert_ref,
                "logo_white": LOGO_WHITE,
                "bosch_logo": BOSCH_LOGO,
                "mileage": mileage_val,
